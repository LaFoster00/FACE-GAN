from keras import models, ops, layers, metrics, losses, random, Variable, backend
import tensorflow as tf

from discriminator import Discriminator
from generator import Generator, Mapping
from src.layers import wasserstein_loss
...
class FaceGAN(models.Model):
    def __init__(self, z_dim=512, target_res=64, start_res=4):
        ...
        self.alpha = Variable(1.0, dtype='float32', trainable=False, name="alpha")

        self.mapping = Mapping(num_stages=self.num_stages, input_shape=z_dim)
        self.d_builder = Discriminator(self.start_res_log2, self.target_res_log2)
        self.g_builder = Generator(self.start_res_log2, self.target_res_log2)
        self.g_input_shape = self.g_builder.input_shape

        self.phase = None
        self.train_step_counter = Variable(0, dtype='int32', trainable=False)
    ...
    def train_step(self, data):
        ...
        if self.phase == "TRANSITION":
            self.alpha.assign(
                ops.cast(self.train_step_counter / self.steps_per_epoch, 'float32')
            )
